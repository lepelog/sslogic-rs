...
use crate::{
    explorer::{Explorer, Placement},
    options::Options,
    requirements::Requirements,
    structure::{ContextLoadable, ItemId, LocationId, LogicContext},
};
...
impl From<LocationId> for LocationOrStart {
...
impl LocationOrStart {
    ...
    pub fn is_location_and(&self, f: impl FnOnce(LocationId) -> bool) -> bool {
        match self {
            Self::Start => false,
    ...
    pub fn is_start_or(&self, f: impl FnOnce(LocationId) -> bool) -> bool {
        match self {
            Self::Start => true,
            Self::Location(location) => f(*location),
        }
    ...
    pub fn name<'a>(&self, ctx: &'a LogicContext) -> &'a str {
        match self {
            Self::Start => "Start",
            Self::Location(loc) => loc.name(ctx)
        }
    }
}
...
impl ItemOrVacant {
    ...
    pub fn is_item_and(&self, f: impl FnOnce(ItemId) -> bool) -> bool {
        match self {
            Self::Vacant => false,
    ...
    pub fn is_vacant_or(&self, f: impl FnOnce(ItemId) -> bool) -> bool {
        match self {
            Self::Vacant => true,
            Self::Item(item) => f(*item),
        }
    ...
    pub fn name<'a>(&self, ctx: &'a LogicContext) -> &'a str {
        match self {
            Self::Vacant => "Vacant",
            Self::Item(item) => item.name(ctx)
        }
    }
}

impl From<ItemId> for ItemOrVacant {
...
impl WeightedItem {
    pub fn simple(item: ItemId) -> Self {
        WeightedItem {
            item: item.into(),
            weight: 1,
        }
...
impl WeightedLocation {
    pub fn simple(location: LocationId) -> Self {
        WeightedLocation {
...
pub fn place_items_plando(
    ...
    options: &Options,
    locations: &mut HashSet<LocationId>,
    items: &mut HashMap<ItemId, u8>,
    progress_locations: &HashSet<LocationId>,
    progress_items: &HashSet<ItemId>,
    ctx: &LogicContext,
) -> Result<(), Error> {
    let mut settings_placed_items: HashSet<ItemId> = HashSet::new();
    let mut settings_filled_locations: HashSet<LocationId> = HashSet::new();
        ...
        for placed_count in 0..count {
                ...
                })
            {
                return Err(Error::SettingsConflictItem {
                    impossible_items: format!(
                        "{}",
                        ctx.display_iter(entry_items.iter().filter_map(|i| i.item.as_item()))
                    ),
                });
            }
                ...
                return Err(Error::SettingsConflictLocation {
                    impossible_locations: format!(
                        ...
                        ctx.display_iter(
                            entry_locations
                                .iter()
                                .filter_map(|l| l.location.as_location())
                        )
                    ),
                });
            }
            if unplaced_items.is_empty() || unfilled_locations.is_empty() {
                ...
                if is_plando {
                    ...
                    println!("{}", ctx.display_iter(unplaced_items.iter().filter_map(|i| i.item.as_item())));
                    println!("{}", ctx.display_iter(unfilled_locations.iter().filter_map(|i| i.location.as_location())));
            ...
            if let Ok(combination) = possible_combinations.choose_weighted(rng, |combo| {
                ...
                println!("placing {} at {}", combination.item.item.name(ctx), combination.location.location.name(ctx));
